...
/* Incrementa salarios El programa incrementa el salario de los empleados de una empresaactualiza el
archivo correspondiente— si sus ventas son superiores al millón de pesos anuales. */
...
rewind(ar);
/* La función rewind se utiliza para posicionarnos en el inicio del archivo cada vez que sea necesario. En este programa no tiene ninguna
...
{
i = ftell(ap) / t;
/* La función ftell se utiliza para conocer la posición de nuestro apuntador en el archivo. La variable i nos proporciona en este caso
el tamaño de todos los bloques que existen debajo de nuestra posición. Si conocemos el tamaño de cada bloque, entonces podemos
...
/* Nos posicionamos nuevamente para leer el siguiente registro. Esta instrucción no debería ser necesaria, pero la función
fwrite se comporta a veces de manera inestable en algunoscompiladores de C. Para asegurarnos que siempre funcione
